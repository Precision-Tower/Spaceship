...
from qps.cipher.reports.report import summarize
from qps.cipher.graph.dependency_closure import build_dependency_closure
from qps.cipher.plan import build_conversion_plan
from qps.cipher.publish import publish_conversion
from qps.cipher.lower.semantic_calls import lower_semantic_calls
...
def _capability_provider():
    try:
        ...
        return EngineeringGeometryProfile().capabilities
    except ImportError:
        return None
...
def write_candidate(
    ...
    *,
    ...
    try:
        output, document = emit_document(source)
