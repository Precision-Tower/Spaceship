...
from Agency.Core.runtime.model_runner import (
    generate_qwen_text,
    generate_qwen_text_with_timeout,
)
from Agency.Core.foundation.shared_bridge import scan_files
...
class TestAllCommand(Command):
    ...
    def run(self, args):
        from Agency.Core.foundation.guardrails import main as guardrails_main
        import sys

        old_argv = sys.argv[:]
        try:
            sys.argv = ["guardrails", "--root", str(self.resolver.resolve_target_root(getattr(args, "root", None)))]
            raise SystemExit(guardrails_main())
        finally:
            sys.argv = old_argv
