...
DASHBOARD_ROOT = CLI_DIR.parents[1]
if str(DASHBOARD_ROOT) not in sys.path:
    sys.path.insert(0, str(DASHBOARD_ROOT))

from Agency.Core.interfaces.cli.commands import apply_patch, cali_observe_directory, clear_latest_diff, create_packet, gemini_analyze, gemini_list_models, git_status, grant_review, grant_review_latest_diff, list_agents, list_packets, propose_diff, propose_directory_diff, runtime_state, scan_repo, test_all, apply_code, read_file, refs, tree
...
import argparse
from Agency.Core.interfaces.cli.commands import (
    view_latest_diff,
)
from Agency.Core.interfaces.cli.registry import propose_task_packet
...
def build_parser():
    ...
    p.set_defaults(func=read_file.run)

    p = sub.add_parser("apply-code")
    p.add_argument("--root", default=None)
    p.add_argument("--file", required=True)
    p.add_argument("--code", required=True)
    p.add_argument("--approved", action="store_true")
    p.set_defaults(func=apply_code.run)

    p = sub.add_parser("refs")
    p.add_argument("query")
    p.add_argument("--root", default=None)
    p.add_argument("--max-files", type=int, default=500)
    p.add_argument("--max-matches", type=int, default=80)
    p.add_argument("--max-bytes", type=int, default=1000000)
    p.add_argument("--raw", action="store_true")
    p.set_defaults(func=refs.run)

    p = sub.add_parser("tree")
    p.add_argument("path")
    p.add_argument("--raw", action="store_true")
    p.set_defaults(func=tree.run)

    return parser


def available_command_names() -> tuple[str, ...]:
    """Return commands that are both dispatchable and authority-registered."""
    parser = build_parser()
    ...
    for action in parser._actions:
        ...
        if choices:
            parser_commands.update(choices.keys())

    return tuple(
        name
        for name in sorted(parser_commands)
        if CommandValidator.is_authorized(name)[0]
    )
...
def run_command(command: str, resolver=None):
    ...
    if resolver is not None:
        for name in (
            ...
            if hasattr(obj, "resolver"):
                obj.resolver = resolver

    return CommandValidator.validate_and_dispatch(args)
