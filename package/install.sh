...
usage_context() {
    pt_print_line "Precision Tower Node package installer"
...
env_conflict_count() {
    ...
    if [[ -f "$PRECISION_ENV_FILE" ]] && ! grep -q '^# Managed by Dashboard Precision Tower package$' "$PRECISION_ENV_FILE"; then
...
write_generated_env() {
    ...
    cat > "$output" <<EOF
# Managed by Dashboard Precision Tower package
...
hardware_gate() {
    ...
    if [[ "$APPLY" == true ]]; then
        pt_status_line "BLOCKED" "hardware readiness" "host is not accepted as a Precision Tower Node; no installation will occur"
        return 1
    fi
