...
for invocation in "${safe_invocations[@]}"; do if bash -lc "$invocation" >"$PT_TEST_TMP_ROOT/precision-tower-command-test.out" 2>&1; then pt_status_line "PASS" "command check" "$invocation"; else cat "$PT_TEST_TMP_ROOT/precision-tower-command-test.out"; fail "command check" "$invocation exited nonzero"; fi; done
rm -f "$PT_TEST_TMP_ROOT/precision-tower-command-test.out"
