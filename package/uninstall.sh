...
pt_print_line "CE-OS Node package uninstaller"
...
if [[ -f "$PRECISION_ENV_FILE" ]] && grep -q '^# Managed by Dashboard CE-OS package$' "$PRECISION_ENV_FILE"; then if [[ "$APPLY" == true ]]; then "${SUDO_CMD[@]}" rm -f "$PRECISION_ENV_FILE"; pt_status_line "REMOVED" "$PRECISION_ENV_FILE"; "${SUDO_CMD[@]}" rmdir "$PRECISION_ENV_DIR" >/dev/null 2>&1 || true; else pt_status_line "DRY-RUN" "$PRECISION_ENV_FILE" "would remove package-managed env file"; fi; elif [[ -e "$PRECISION_ENV_FILE" ]]; then pt_status_line "PRESERVED" "$PRECISION_ENV_FILE" "not package-managed"; else pt_status_line "SKIPPED" "$PRECISION_ENV_FILE" "not installed"; fi
