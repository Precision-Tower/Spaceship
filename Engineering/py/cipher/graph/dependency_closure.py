...
def project_dependency_destinations(
    ...
    """Project every local closure unit beside the entry destination.

    The common directory containing the complete local closure is
    mapped onto the corresponding ancestor of the entry destination.
    Source topology beneath that root is preserved.
    """
    destination = Path(entry_destination)

    unit_paths = list(closure.units)
    ...
    entry_relative = closure.entry.relative_to(
        common_source_root
    )
    ...
    for source in unit_paths:
        ...
        if target.name == "__init__.py":
            target = target.with_name("_index.qps")
        else:
            target = target.with_suffix(".qps")
